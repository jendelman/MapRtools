...
EGQ <- function(depth,error,ploidy) {
  
  Likelihood <- function(k,N,error,ploidy){
    p <- seq(0,1,by=1/ploidy)
    q <- 1-p
    z <- dbinom(x=k,size=N,prob=p*(1-error)+q*error)
    names(z) <- 0:ploidy
    return(z/sum(z))
  }
  
  f1 <- function(N,error,ploidy) {
    k <- seq(0,N,by=1)
    p <- apply(array(k),1,Likelihood,N=N,error=error,ploidy=ploidy)
    geno.mode <- apply(p,2,which.max)-1
    prob.error <- sum(dbinom(k,N,prob=0.5)[which(geno.mode!=ploidy/2)])
    return(-10*log10(prob.error))
  }
  min(apply(array(depth:(depth+10)),1,f1,error=error,ploidy=ploidy))
}
